...
def test_get_top_uploaders(mock_db):
    ...
    mock_db.query.return_value.join.return_value.group_by.return_value.order_by.return_value.limit.return_value.all.return_value = mock_result

    result = user_stats_services.get_top_uploaders(limit=3, db=mock_db)
